...
from helpers_GTSRB import *


class AttackCarliniWagnerL2:

    def __init__(self, targeted=False, search_steps=1000, max_steps=None, cuda=True, debug=False):
        self.debug = debug
        self.targeted = targeted
        self.num_classes = 43
        self.confidence =29 #(14 is better)# FIXME need to find a good value for this, 0 value used in paper not doing much...
        self.initial_const = 0.1  # bumped up from default of .01 in reference code
        self.binary_search_steps = 5
        self.repeat = self.binary_search_steps >= 10
        self.max_steps = search_steps
        self.abort_early =False
        self.clip_min = -1.
        self.clip_max = 1.
        self.cuda = cuda
        self.clamp_fn = None#'tanh'  # set to something else perform a simple clamp instead of tanh
    ...
    def _optimize(self, optimizer, model, input_var, modifier_var, target_var, scale_const_var, input_orig=None):
        ...
        model, input_var, modifier_var, target_var, scale_const_var=model.cuda(), input_var.cuda(), modifier_var.cuda(), target_var.cuda(), scale_const_var.cuda()
    ...
    def run(self, model, input, target, batch_idx=0):
        ...
        target_onehot.scatter_(1, target.unsqueeze(1).cuda(), 1.)
            ...
            for i in range(batch_size):
                if self._compare(best_score[i], target[i]) and best_score[i] != -1:
                    ...
                    if upper_bound[i] < 1e9:
                        ...
                        scale_const[i] *= 10
                    if self.debug:
                        print('{0:>2} failed attack, raising const to {1:.3f}'.format( i, scale_const[i]))
