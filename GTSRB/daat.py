...
def margin_loss(label, features, num_classes, alpha=0.1,marker=0,centers_old=0):
    ...
    centers=Variable((torch.zeros(num_classes,lenf_features))).cuda()
    unique_n=[]
    unique_n=np.asarray(unique_n,np.float32)
    tmp_label = Variable(torch.from_numpy(np.zeros((len(label),1),np.int64))).cuda()
    ...
    for j in range(len(features)):
        centers[int(label.cpu().data.numpy()[j])].data=centers[int(label.cpu().data.numpy()[j])].data+features[j].data
    unique_n=Variable(torch.from_numpy(unique_n))
    unique_nn=Variable(torch.from_numpy(np.asarray(unique_n.clone().repeat(1, lenf_features).data.cpu().numpy(),np.float32))).cuda()
    ...
    label=Variable(torch.from_numpy(np.asarray(label.data.cpu().numpy(),np.int64))).view(-1,).cuda()
